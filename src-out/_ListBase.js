    ...
    var __class = declare('argos._ListBase', [View, _PullToRefreshMixin], {
        ...
        */
        ...
        selectEntrySilent: function (key) {
            var enableActions = this.enableActions, selectionModel = this.get('selectionModel'), selectedItems, selection, prop;
            if (key) {
        ...
        invokeActionItem: function (parameters, evt, node) {
            var index = parameters['id'], action = this.visibleActions[index], key, selectedItems = this.get('selectionModel').getSelections(), selection = null;
        ...
        selectEntry: function (params, evt, node) {
            var row = query(node).closest('[data-key]')[0], key = row ? row.getAttribute('data-key') : false;
        ...
        navigateToRelatedView: function (action, selection, viewId, whereQueryFmt, additionalOptions) {
            var view = this.app.getView(viewId), options = {
        ...
        navigateToDetailView: function (key, descriptor, additionalOptions) {
            var view = this.app.getView(this.detailView), options = {
        ...
        navigateToEditView: function (action, selection, additionalOptions) {
            var view = this.app.getView(this.editView || this.insertView), key = selection.data[this.idProperty], options = {
                key: key,
                selectedEntry: selection.data,
                fromContext: this
            };
        ...
        navigateToInsertView: function (el, additionalOptions) {
            var view = this.app.getView(this.insertView || this.editView), options = {
                returnTo: this.id,
                insert: true
            };
        ...
        requestData: function () {
            ...
            if (store) {
                ...
                when(queryResults, this._onQueryComplete.bind(this, queryResults), this._onQueryError.bind(this, queryOptions));
                return queryResults;
            }
        ...
        processData: function (entries) {
            ...
            var store = this.get('store'), rowNode, output, docfrag, key, entry, i, count = entries.length;
            if (count > 0) {
                output = [];
