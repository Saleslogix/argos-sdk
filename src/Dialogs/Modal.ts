...
const __class = declare('argos.Dialogs.Modal', [_WidgetBase, _Templated], {
  ...
  */
  add: function add(content: any = {}, toolbarActions = [], options = {}) {
  ...
  createSimpleAlert: function createSimpleAlert(options: any = {}) {
    const dialog = {
  ...
  createSimpleDialog: function createSimpleDialog(options: any = {}) {
    const dialog = {
      title: this.defaultHeaderText[options.title] || options.title,
      content: options.content,
      getContent: options.getContent,
  ...
  */
  ...
  resolveDeferred: function resolveDeferred() {
    ...
    if (this._content && this._content.getContent) {
      ...
      if (this.isNotSimpleDialog()) {
        console.log('Modal content does not have a getContent function call to retrieve the data, add this to allow data to be returned'); // tslint:disable-line
      }
    }
  ...
  updateZIndex: function updateZIndex(above) {
    if (above) {
      let value = parseInt($(above).css('zIndex'), 10);
      if (!value) {
        value = 0;
      }
