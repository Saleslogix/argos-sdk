...
const __class = declare('argos.RelatedViewWidget', [_RelatedViewWidgetBase, _CustomizationMixin, _ActionMixin], {
  ...
  relatedViewFooterTemplate: new Simplate([
    ...
    '<div  data-dojo-attach-point="navtoListFooterNode" class="action" data-dojo-attach-event="onclick:onNavigateToList">{%: $$.navToListText %}</div>', // tslint:disable-line
    ...
    if (this.relatedData) {
      ...
      ((context, relatedResults) => {
        try {
          ...
          console.log('Error fetching related view data:' + error); // tslint:disable-line
        }
  ...
  onApply: function onApply(relatedFeed) {
    try {
      if (!this.itemsNode) {
        this.itemsNode = $("<div id='itemsNode' class='items'><div>"); // tslint:disable-line
      ...
      console.log('Error applying data for related view widget:' + error); // tlint:disable-line
    }
  },
