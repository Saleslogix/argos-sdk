...
const __class = declare('argos.Views.FileSelect', [View], {
  ...
  widgetTemplate: new Simplate([
    ...
    '<div class="buttons">',
    '<div><button id="fileSelect-btn-upload" data-dojo-attach-point="btnUploadFiles" class="btn-primary" data-action="onUploadFiles"><span>{%: $.uploadText %}</span></button>', // tslint:disable-line
  ...
  _getFileLength: function _getFileLength(file) {
    ...
    if (filelength === 0) {
      filelengthText += `0 ${this.bytesTextBytes}`;
    } else {
      if (filelength) {
        if (filelength > 1024) {
          if (filelength > 1048576) {
            filelengthText = `${Math.round(filelength / 1048576)} MB`;
          } else {
            filelengthText = `${Math.round(filelength / 1024)} KB`;
          }
        } else {
          filelengthText += ` ${this.bytesTextBytesBytes}`;
        }
      }
    }
