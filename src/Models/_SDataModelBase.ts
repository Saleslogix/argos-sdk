...
const __class = declare('argos.Models.SDataModelBase', [_ModelBase], {
  ...
  getOptions: function getOptions(options) {
    ...
    if (options) {
      if (options.select) {
        tempOptions.select = options.select;
      }
      if (options.include) {
        tempOptions.include = options.include;
      }
      if (options.orderBy) {
        tempOptions.orderBy = options.orderBy;
      }
      if (options.contractName) {
        tempOptions.contractName = options.contractName;
      }
      if (options.resourceKind) {
        tempOptions.resourceKind = options.resourceKind;
      }
      if (options.resourceProperty) {
        tempOptions.resourceProperty = options.resourceProperty;
      }
      if (options.resourcePredicate) {
        tempOptions.resourcePredicate = options.resourcePredicate;
      }
      if (options.queryArgs) {
        tempOptions.queryArgs = options.queryArgs;
      }
      if (options.start) {
        tempOptions.start = options.start;
      }
      if (options.count) {
        tempOptions.count = options.count;
      }
    }
  ...
  buildQueryExpression: function _buildQueryExpression(query, options) {
    ...
    return passed
      ? query
        ? '(' + utility.expand(this, passed) + ') and (' + query + ')'
  ...
  getRelatedQueryOptions: function getRelatedQueryOptions(entry, relationship, options) {
    ...
    const where = "${0} eq '${1}'"; // tslint:disable-line
    if (!relatedValue) {
      return null;
    }
