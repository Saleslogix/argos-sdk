 ...
 * Field is the base class for all field controls. It describes all the functions a field should
 * support giving no implementation itself, merely a shell. The one function that `_Field` does
 * provide that most fields leave untouched is `validate`.
 *
...
const __class = declare('argos.Fields._Field', [_WidgetBase, _ActionMixin, _Templated], {
   ...
   *    * If the validator is a function, call and return the result of the function passing the value, _Field instance,
   *      and the `owner` property.
   ...
   *   * Otherwise, assume it is a string format and call dojo's `string.substitute` using the message as the format, `${0}` as
   *      the value, `${1}` as the fields name, `${2}` as the fields label property.
   ...
   * @return {Boolean/Object} False signifies that everything is okay and the field is valid, `true` or a `string message`
