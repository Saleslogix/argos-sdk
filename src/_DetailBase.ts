...
const __class = declare('argos._DetailBase', [View, TabWidget], {
  ...
  widgetTemplate: new Simplate([
    '<div id="{%= $.id %}" title="{%= $.titleText %}" class="detail panel scrollable {%= $.cls %}" {% if ($.resourceKind) { %}data-resource-kind="{%= $.resourceKind %}"{% } %}>', // tslint:disable-line
    '{%! $.loadingTemplate %}',
    '{%! $.quickActionTemplate %}',
  ...
  sectionBeginTemplate: new Simplate([
    ...
    '<button class="{% if ($.collapsed) { %}{%: $$.toggleExpandClass %}{% } else { %}{%: $$.toggleCollapseClass %}{% } %}" aria-label="{%: $$.toggleCollapseText %}"></button>', // tslint:disable-line
    '{%: ($.title || $.options.title) %}',
    '</h2>',
    '{% } %}',
  ...
  relatedPropertyTemplate: new Simplate([
    ...
    '<label>{%: $.label %}</label>',
    '<span class="data">',
    '<a class="hyperlink" data-action="activateRelatedEntry" data-view="{%= $.view %}" data-context="{%: $.context %}" data-descriptor="{%: $.descriptor || $.value %}">', // tslint:disable-line
    '{%= $.value %}',
    '</a>',
    '</span>',
    '</div>',
  ...
  relatedTemplate: new Simplate([
    '<li class="relatedviewitem {%= $.cls %}" data-action="activateRelatedList" data-view="{%= $.view %}" data-context="{%: $.context %}" {% if ($.disabled) { %}data-disable-action="true"{% } %}>', // tslint:disable-line
  ...
  actionPropertyTemplate: new Simplate([
    ...
    '<label>{%: $.label %}</label>',
    '<span class="data">',
    '<a data-action="{%= $.action %}" {% if ($.disabled) { %}data-disable-action="true"{% } %} class="{% if ($.disabled) { %}disabled{% } %}">', // tslint:disable-line
    '{%= $.value %}',
    '</a>',
    '</span>',
    '</div>',
  ...
  createErrorHandlers: function createErrorHandlers() {
    this.errorHandlers = this.errorHandlers || [{
      ...
      handle: (error, next) => {
        alert(this.getErrorMessage(error)); // tslint-disable-line
        next();
      },
    }, {
      name: 'NotFound',
   ...
   *        onCreate: function(), // Optional. You may pass a function to be called when the row is
   *        added to the DOM
   ...
   *        template: Simplate, // Optional. Override the HTML Simplate used for rendering the
   *        value (not the row) where `$` is the row object
   ...
   *        provider: function(entry, propertyName), // Optional. Function that accepts the data
   *        entry and the property name and returns the extracted value. By default simply extracts directly.
  ...
  processLayout: function processLayout(layout, entry) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (current.descriptor) {
        data.descriptor = typeof current.descriptor === 'function' ? this.expandExpression(current.descriptor, entry, value) : provider(entry, current.descriptor); // tslint:disable-line
      }
    ...
    const renderRelated = (result) => {
      if (result >= 0) {
        ...
        if (labelNode.length) {
          ...
          console.warn('Missing the "related-item-label" dom node.'); // tslint-disable-line
        }
      }
    };
