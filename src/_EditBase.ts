...
const __class = declare('argos._EditBase', [View], {
  ...
  widgetTemplate: new Simplate([
    '<div id="{%= $.id %}" title="{%: $.titleText %}" class="edit panel scrollable {%= $.cls %}" {% if ($.resourceKind) { %}data-resource-kind="{%= $.resourceKind %}"{% } %}>', // tslint:disable-line
    '{%! $.loadingTemplate %}',
  ...
  propertyTemplate: new Simplate([
    ...
    '<div class="row-edit {%= $.cls %}{% if ($.readonly) { %}row-readonly{% } %}" data-field="{%= $.name || $.property %}" data-field-type="{%= $.type %}">', // tslint:disable-line
    '</div>',
  ]),
  ...
  getContext: function getContext() {
    return lang.mixin(this.inherited(arguments), {
      ...
      key: this.options.insert ? false : this.options.key ? this.options.key : this.options.entry && this.options.entry[this.idProperty] // tslint:disable-line
    });
  },
  /**
  ...
  onTransitionTo: function onTransitionTo() {
    ...
    // This flag is important because onTransitionTo will fire multiple times if the
    // user is using lookups and editor type fields that transition away from this view.
